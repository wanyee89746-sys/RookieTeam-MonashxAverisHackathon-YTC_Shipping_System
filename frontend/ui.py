...
def render_filters(emails):
    ...
    with top_b:
        ...
        if raw:
            ...
            st.caption(
                "Replays saved pipeline output (precomputed)."
            )
    ...
    if raw:
        ...
        search = c_search.text_input(
            "Search",
            placeholder=(
                "Search email ID, subject, or sender..."
            ),
        )
        ...
        c_search, c_status, c_cat, c_flow, c_sort = (
            st.columns(
                [2.2, 1, 1, 1, 1]
            )
        )
...
def render_inbox_list(
    ...
    st.caption(
        f"Showing {len(filtered)} of {total} emails"
    )
...
def _render_raw(email_id):
    ...
    st.info(
        "Not processed yet. Click **▶ Run pipeline** "
        "in the inbox to see the result."
    )
    ...
    if not atts:
        ...
        st.warning(
            "No attachments. A comparison request "
            "without SI/BL can't be checked."
        )
...
def _render_ai_recovery(report):
    """
    Show Vision LLM recovery information.

    This is technical extraction information only.
    It does NOT change the official verification status.
    """
    ...
    # Nothing to show when Vision was not used.
    if not (
        bl_vision_used
        or si_vision_used
    ):
    ...
    st.caption(
        "Vision LLM was used because standard document "
        "text extraction could not read the PDF."
    )
    ...
    if bl_vision_used:
        ...
        if bl_vision_success:
            ...
            st.success(
                "✅ BL document content was successfully "
                "recovered using Vision LLM."
            )

        else:

            st.error(
                "❌ Vision LLM could not recover the "
                "BL document."
            )
    ...
    if si_vision_used:
        ...
        if si_vision_success:
            ...
            st.success(
                "✅ SI document content was successfully "
                "recovered using Vision LLM."
            )
    ...
    # ---------------------------------------------------------
    # Important explanation for judges
    # ---------------------------------------------------------

    st.info(
        "The Vision LLM recovery is shown separately from "
        "the verification result. The original verification "
...
def _render_review(
    ...
    if (
        report.get("reviewed")
        and report.get("original_status")
    ):
    ...
    with st.expander(
        ...
        c1.caption(
            f"SI: "
            f"{ev.get('si_path') or 'no attachment'}"
        )
        ...
        c2.caption(
            f"BL: "
            f"{ev.get('bl_path') or 'no attachment'}"
        )
    ...
    st.caption(
        "Edit any SI or BL value, then save "
        "to recompute the result."
    )
...
def render_report(email):
    ...
    if status == "CLASSIFIED_ONLY":
        ...
        st.info(
            ...
            "Only BL comparison requests go through "
            "the document check."
        )
...
def _render_comparison_table(
    ...
    for field in FIELD_LABELS:
        ...
        outcome = field_results.get(
            field
        )

        if (
            field in defect_fields
            or outcome == "MISMATCH"
        ):
