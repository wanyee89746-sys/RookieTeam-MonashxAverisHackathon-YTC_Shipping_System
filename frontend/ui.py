...
def _render_ai_recovery(
    email_id,
    report,
):
    """Show actual Vision LLM recovered text for unreadable documents."""
    ...
    # Get the actual recovered text from the backend.
    evidence = fetch_evidence(email_id)

    si_text = evidence.get("si_text") or ""
    bl_text = evidence.get("bl_text") or ""

    st.markdown("### 🤖 AI Recovery")

    st.info(
        "Standard document extraction could not read this document. "
        "Vision LLM recovered the following information for review."
    )
    ...
    with col1:
        ...
        if si_vision and si_text:
            st.success(
                "✅ Vision LLM recovery successful"
            )
            ...
            st.text_area(
                ...
                height=280,
                disabled=True,
                key=f"ai_si_{email_id}",
                label_visibility="collapsed",
            )

        elif si_vision:
            st.warning(
                "Vision LLM recovery was successful, "
                "but no recovered text is available."
            )

        else:
            st.warning(
                "Vision LLM recovery not successful."
            )
    ...
    st.caption(
        "The official verification result remains unchanged. "
        "AI recovery provides additional evidence for human review."
    )
